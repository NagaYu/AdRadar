...
> - ⚠️ **The scraper has not been validated against the live Ad Library in this release.** The DOM-extraction selectors in [`src/scraper.ts`](src/scraper.ts) are written against Meta's *expected* markup. Meta's Ad Library is a heavily obfuscated, frequently-changing SPA — it may return zero results behind region/login gates, or rename the text anchors the extractor relies on, **at any time and without notice**. Treat a clean run as something to verify, not assume.
> - 🩹 **Expect to maintain the selectors.** When Meta changes its DOM, update the regexes/anchors in `extractAds`, `dismissCookieBanner`, and `infiniteScroll`. This is inherent to scraping, not a bug.
